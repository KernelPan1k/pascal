...
const baseNavItems = [
  { href: "/panneau", label: "Dashboard", icon: "◈" },
  { href: "/panneau/articles", label: "Articles", icon: "✍" },
  { href: "/panneau/discographie", label: "Discographie", icon: "♬" },
  { href: "/panneau/pages", label: "Pages", icon: "☰" },
  { href: "/panneau/media", label: "Médias", icon: "◉" },
  { href: "/panneau/temoignages", label: "Témoignages", icon: "✦" },
];

const adminOnlyItems = [
  { href: "/panneau/users", label: "Utilisateurs", icon: "◎" },
  { href: "/panneau/settings", label: "Paramètres", icon: "⚙" },
];
...
export default function AdminSidebar({ userRole, userName }: Props) {
  ...
  const isActive = (href: string) => {
    if (href === "/panneau") return pathname === "/panneau";
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <button
            onClick={() => signOut({ callbackUrl: "/panneau/login" })}
            style={{
              background: "none",
              border: "none",
              fontSize: "0.7rem",
